...
本组件认为，一个视觉上的页面由三个重要部分组成：页面（page-header）、页脚（page-footer）和页面主体（page-body）。本组件同时提供此三个组成部分。并且，页面与页脚为可选配件，即可通过输入项（Vue 组件之 prop）配置二者存在与否。页面主体之存在性不可配置，即，页面主体恒存在。

本组件要解决一些用程序迎合美术设计思路之痛点。具体而言，常见之页面设计之布局有若干种，每种设计对应的程序实现均有所区别。本工具之设计目的即是一应俱全（流行语所谓“提供‘一站式’解决方案”），令采用者单用这一个可配置的“页面”组件，就可灵活应对多种设计要求。本组件所能满足的设计形式不妨列举如下：
...
- 【页眉】是否总是覆盖【页面主体】。这一要求往往为了美观，且【页眉】本身会有半透明底色，以使衬于其下的【页面主体】若隐若现。

- 【页脚】是否总是覆盖【页面主体】。这一要求往往为了美观，且【页脚】本身会有半透明底色，以使衬于其下的【页面主体】若隐若现。
...
- 即便【页脚】不悬挂于页面底部，本租金仍允许将【页脚】配置为“覆盖于【页面主体】之上层”。参见上一条目。

- 为防止覆盖在【页面主体】上层的【页眉】遮挡【页面主体】之正文内容，本组件会自动根据【页眉】之高度，为必要的页面构成元素设置准确的 `padding-top`。采用本组件之程序无需担心这些细节。

- 为防止覆盖在【页面主体】上层的【页脚】遮挡【页面主体】之正文内容，本组件会自动根据【页脚】之高度，为必要的页面构成元素设置准确的 `padding-bottom`。采用本组件之程序无需担心这些细节。
...
1.  **所谓“关键但不确定”之样式的运用。**
    ...
    这是指一些虽然必不可少，但却因项目而异的样式配置。典型的例子是【页眉】之高度、【页脚】之高度。于此两数值，本组件不便冒然给出所谓默认值，故而有意空缺，留待本组件之外的代码补充给出。换而言之，诸君采用本组件时，莫要忘记自行定义这些样式，以获得正确的效果。还需指出，于这部分样式之定义、配置，本组件有推荐做法。参阅下文《[所谓“关键但不确定”的样式](#%E6%89%80%E8%B0%93%E2%80%9C%E5%85%B3%E9%94%AE%E4%BD%86%E4%B8%8D%E7%A1%AE%E5%AE%9A%E2%80%9D%E7%9A%84%E6%A0%B7%E5%BC%8F)》一节。

1.  **非必须的所谓“额外的样式预设”之运用。**

    这些额外样式视同所谓“预设”，系为达成某些习惯提供方便而已。这些样式亦由本组件附带，可供选用。它们**不**存在于 `.vue` 文件中，而是存在于一些独立的 `.css` 中。
...
-   默认插槽，被 `div.page-body-content` 包裹，用于接纳页面主体内容。
-   名为 `page-header-content` 的插槽，被 `div.page-header` 包裹。功能不言自明。
-   名为 `page-footer-content` 的插槽，被 `div.page-footer` 包裹。功能不言自明。
-   名为 `before-page-body-content` 的插槽，被 `div.before-page-body-content` 包裹。功能不言自明。
-   名为 `after-page-body-content` 的插槽，被 `div.after-page-body-content` 包裹。功能不言自明。
...
<template>
    ...
    >
        ...
        <div class="my-first-page-body-content">
            ...
            <hr>
            <dl>
                <dt>复旦大学中国研究院院长张维为教授在其所著《中国超越：一个文明型国家的光荣与梦想》中写道：</dt>
                <dd>
                    <p>中国人的爱国主义
                        是长江、黄河、珠穆朗玛峰；
                        是诗经、楚辞、先秦散文；
...
当然，类似 Nuxtjs 这样的应用平台，会在 `#app` 之上层额外插入更多的 `div`。此时，这些额外的祖先 `div` 显然超出本组件的掌控范围。换句话说，本组件的相关尝试可能恰巧（恰不巧？）无如期效果。而于是情形，本组件无可奈何。但本组件会在浏览器控制台（`console`）中打印警告信息，以提示诸君手工做额外的配置。**而诸君须做的无非是确保 HTML 层级中自 `<html>` 以来的所有祖先元素均提供准确的 `height` 或 `min-height`。** 仅此而已。
...
诸君如果好奇幕后起作用之代码，可参考下方的源代码片段。更多细节，尊请阅读 `./source/index.vue` 中样式块的源代码。
...
为多个项目、甚至为统一项目的多个页面反复书写额外样式，未免乏味，甚至令人厌烦。于是，本组件亦附带了两种预设好的 `.css` 文件（共三个），以方便地实现符合本人喜好的样式。诸君若有兴趣，欢迎尝试。
...
.page {
    ...
    --page-footer-bg-color-with-backdrop-filter    rgba(black, 0)
    --page-footer-bg-color-without-backdrop-filter rgba(black, 0)
}
```

诚然，你可以故意避免采用这 6 个 CSS 变量，改为直接为 `.page-header`、`.page-footer` 设置 `background-color` 和 `backdrop-filter`。而采用上述 6 个 CSS 变量之微弱的好处是，幕后所涉及的 CSS 代码会确保毛玻璃效果仅在有必要时得到采用，且总是自动据此选用对应的背景色方案。仅此而已。
...
2.  为所谓页面内容配置统一的水平方向的 `padding`，即 `padding-left` 和 `padding-right`。
...
不论本组件【根 `div` 】的直接父元素是否为 `#app`，诸君总是必须为该直接父元素添加一个 CSS 类名——`.page-container`。即有：
