export function checkoutPageContainerElementAncestor(pageRootElement, pageContainerElement, shouldDisableWarning) {
    if (!pageContainerElement) {
        return
    }
    const layoutMightHaveIssue = pageContainerElement.parentElement !== document.body
...
export function setupPageAncestorElements(pageContainerElement, pageMinHeightShouldFitScreen) {
    if (!pageContainerElement) {
    ...
    if (!pageContainerElement.classList.contains(cssClassNameToCheck)) {
        console.error('The direct parentElement of a Page, which is\n', pageContainerElement, `\nmust have the CSS class name "${cssClassNameToCheck}".`)
    }
    document.documentElement.classList.add('has-page')
    updatePageContainerElementCSSClassNames(pageContainerElement, pageMinHeightShouldFitScreen)
}
export function restoreAncestorElements() {
...
export function updatePageContainerElementCSSClassNames(pageContainerElement, pageMinHeightShouldFitScreen) {
    if (!pageContainerElement) {
        return
    }
    ;
    [
        ...
        elment.classList.toggle('page-height-should-fit-screen', !!pageMinHeightShouldFitScreen)
    })
}
